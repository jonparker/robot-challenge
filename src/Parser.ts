...
const parseDirection = (direction: string): Compass | Error => {
    ...
    return (
        mappings[direction] ?? Error(`Could not parse direction ${direction}`)
    )
}

const parseInitialLocationTokens = (
    direction: string,
    xStr: string,
    yStr: string
): Location | Error => {
    ...
    return !isNaN(x) && !isNaN(y) && !isError(orientation)
        ? { x, y, orientation }
        : isError(orientation)
        ? orientation
        : Error(`Could not parse ${xStr} or ${yStr}`)
}
...
const parseRobotCommand = (command: string, repeat: number): RobotCommand =>
    ...
    } as Record<string, Rotate | Move>)[command.toUpperCase()])
...
const parseInitialLocation = (initialLocationStr: string): Location | Error => {
    ...
    const initialLocation = parseInitialLocationTokens(
        initialDirectionStr,
        x,
        y
    )
    return isError(initialLocation)
        ? Error(
              `Initial location ${initialLocationStr} could not be parsed: ${initialLocation}`
          )
        : initialLocation
}
